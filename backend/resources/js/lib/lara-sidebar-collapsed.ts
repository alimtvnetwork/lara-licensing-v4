...
function writeValue(next: boolean): void {
  ...
  catch (err) { console.warn("LaraSidebarCollapsed persist failed", err); }
...
export function useSidebarCollapsed(): [boolean, () => void] {
  ...
  useEffect(() => {
    ...
    } catch (err) { console.warn("LaraSidebarCollapsed read failed", err); }
    setHydrated(true);
  }, [hydrated]);
  const toggle = () => { writeValue(!currentValue); };
  return [value, toggle];
}
