# 50 number of steps plan, maximum enforcement (v4.1)

## RULE 0, step count is law

Produce EXACTLY `50` steps. Not `50-1`, not `50+1`. `50` is a positive integer injected at runtime. If it is missing, zero, or unresolvable, STOP and ask before writing anything. Count the steps twice before saving.
...
- Step count other than exactly `50`
...
- [ ] `50` resolved (integer > 0); read this prompt end-to-end
...
- [ ] EXACTLY `50` steps, counted twice; each concrete, verifiable, referencing its spec task file; no filler
