...
Application variables are defined as **CSS custom properties** in `src/styles.css` within `:root` (light) and `.dark` (dark) blocks. Values use OKLCH, and `@theme inline` maps semantic variables to Tailwind v4 utilities.
...
Every application CSS custom property is mapped to a Tailwind utility class in `src/styles.css`:
