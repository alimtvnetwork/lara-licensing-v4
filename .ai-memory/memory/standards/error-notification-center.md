...
1. **Client-side Store:** Errors are normalized via `error-store.ts` into a bounded ring buffer (50 entries) per 02-spec/03-error-manage. This buffer persists across page reloads via `sessionStorage`.
