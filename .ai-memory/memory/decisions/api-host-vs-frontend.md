...
`02-spec/21-app/` names the product `LaraLicensingV1` and specifies endpoints
...
schema in `02-spec/23-app-db/01-schema.md`).
...
2. This TanStack Start repo is the **operator UI** for LaraLicensingV1,
   ...
   `02-spec/21-app/04-roles.md`. The EndUser device client is out of scope.
...
4. Authentication uses the JWT flow in `02-spec/21-app/02-authentication-jwt.md`.
...
- The frontend needs one HTTP client module that: reads
  ...
  `02-spec/21-app/11-api-contracts/00-overview.md`, maps `ErrorCode` values
  from `02-spec/21-app/12-error-taxonomy.md`, and surfaces 429 headers per
