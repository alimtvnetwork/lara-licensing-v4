...
1. No files at the `.ai-memory/memory/` root. Every memory file lives under a topic folder: `.ai-memory/memory/<topic>/XX-<slug>.md`.

2. Path is `.ai-memory/memory/`, never `.ai-memory/memories/`. Path is `.ai-memory/plans/`, never `plan/`. Path is `.ai-memory/ambiguous-questions/`, never `ambiguity/`.
...
7. Plans and suggestions single-file trackers stay single files: `.ai-memory/plan.md` (or `.ai-memory/plans/index.md` for the roll-up), `.ai-memory/suggestions.md`. Per-suggestion verbatim captures live under `.ai-memory/suggestions/XX-<slug>.md`.
