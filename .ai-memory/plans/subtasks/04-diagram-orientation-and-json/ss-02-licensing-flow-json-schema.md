...
## Purpose
Define the JSON companion for `02-spec/21-app/diagrams/licensing-flow.mmd` so tooling and downstream AIs consume the communication flow without parsing Mermaid.

## Filename
`02-spec/21-app/diagrams/licensing-flow.json` (colocated, searchable by name).
...
```
{
  ...
  "Invariants": [
    ...
    "AuditLog receives { RequestId, ActorUserId, Action, Decision } per 02-spec/21-app/22-log-line-contract.md"
  ]
}
```
