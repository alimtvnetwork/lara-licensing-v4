...
Author `02-spec/25-app-audit/00-verdict-plain.md` so a non-auditor reader can, in under two minutes, learn: the current score, what it means, what the top three residual risks are, and where in `02-spec/21-app/` those risks live. Link it from `02-spec/21-app/00-overview.md` and the root `README.md`.
...
- `02-spec/21-app/00-overview.md` has a "Verdict" link in the header.
