...
2. Log drift in `02-spec/25-app-audit/06-lara-lib-drift-report.md` (create if missing): file, operationId, drift kind (`missing-field` / `extra-field` / `wrong-casing` / `wrong-type` / `any-leak`).
