...
Execute the 30-file spec authoring cycle for `02-spec/26-backup-restore/`
outlined in `.ai-memory/plans/pending/12-backup-restore-snapshot.md`. This
plan writes the specification only, no runtime code. Each step lands
exactly one authoritative markdown artifact in `02-spec/26-backup-restore/`
...
Captured inputs:
- Command: `.ai-memory/02-spec/commands/08-backup-restore-spec-30-steps.md`
- Parent outline: `.ai-memory/plans/pending/12-backup-restore-snapshot.md`

Appended prior pending items (see final section).

## Steps

1. Create `02-spec/26-backup-restore/` and write `00-overview.md` (purpose, non-goals, glossary, actors, threat-model summary).
...
30. Write `99-consistency-report.md` (forbidden-terms scan, closed-set parity, permission-matrix vs policy.csv parity, cross-file link check) and MOVE plan 12 + this plan file to `.ai-memory/plans/completed/` with `Status: completed`.

## Verification

- After each step: file exists at declared path, front-matter version bumped in `02-spec/26-backup-restore/98-changelog.md` at step 29.
- Cross-file link check: `rg -n '\]\(\.\./' 02-spec/26-backup-restore/` resolves.
- Closed-set parity: BR error codes present in both `02-spec/03-error-manage/` catalog and `backend/config/lara.php` list at step 17.
- Permission matrix vs Casbin policy.csv: consistency report (step 30) diff is empty.
- Plan lifecycle: after step 30, both `12-backup-restore-snapshot.md` and `13-backup-restore-spec-authoring.md` live under `.ai-memory/plans/completed/` with `Status: completed`; no duplicates in `pending/`.
