...
- [x] Playwright sandbox blocker documented in 02-spec/25-app-audit/06-plan16-step97-playwright-sandbox-blocker.md; green run routed to CI.
...
Captured command: `.ai-memory/02-spec/commands/09-preview-vs-production-mode-and-typed-api.md`.
Captured issue: `.ai-memory/issues/04-preview-cannot-exercise-ui-without-backend.md`.
Coding guidelines in force: `02-spec/02-coding-guidelines/` (15-line function cap, PascalCase JSON keys, no magic literals, closed-set enums).
Error-manage rules in force: `02-spec/03-error-manage/` (LaraException envelope, RequestId/ErrorId correlation, no silent failure, retry-after propagation).
Existing typed API surface to audit: `src/lib/lara-*.ts` (20+ files).

## Steps

1. Author `02-spec/28-runtime-modes/00-overview.md` defining the three modes (`preview`, `dev`, `production`), transitions, and invariants (`INV-RM-*`).
2. Author `02-spec/28-runtime-modes/01-version-json-schema.md` with the JSON Schema for repo-root `version.json` (`version`, `mode`, `apiBaseUrl`, `previewSeed`, `updatedAt`, `allowRuntimeToggle`).
3. Author `02-spec/28-runtime-modes/02-mode-selection-precedence.md`: precedence is `localStorage override (admin toggle) -> version.json -> compile-time default`.
4. Author `02-spec/28-runtime-modes/03-preview-fixture-contract.md` describing how every endpoint MUST have a preview handler with seed data.
5. Author `02-spec/28-runtime-modes/04-generated-types-contract.md`: how OpenAPI produces `src/generated/api/`, banned edits, regeneration workflow.
6. Author `02-spec/28-runtime-modes/05-admin-runtime-toggle.md`: RBAC (root only), audit log, persistence rules.
7. Author `02-spec/28-runtime-modes/06-acceptance-criteria.md` with `AC-RM-01..AC-RM-25` covering all screens listed in Step 66.
8. Author `02-spec/28-runtime-modes/07-open-questions.md` for unresolved items (persistence in preview: IndexedDB vs in-memory, etc).
...
79. Add error-contract parity: preview handlers must emit exactly the closed-set error codes registered in `02-spec/03-error-manage/03-error-code-registry`. Add a test that walks all handlers and validates codes.
...
85. Document the preview scenarios in `02-spec/28-runtime-modes/08-preview-scenarios.md`.
86. Add screenshots to `02-spec/28-runtime-modes/screenshots/` showing every route rendered in preview mode (default seed, empty seed, error seed).
...
91. Update `.ai-memory/plans/pending/06-laravel-be-fe-and-publish.md` cross-reference to depend on this plan's runtime-toggle endpoint.
92. Add cross-refs from `02-spec/03-error-manage/` to preview handlers as fixtures for the error-code test bench.
93. Add cross-refs from `02-spec/07-design-system/` empty-state guidance to `src/lib/preview-seeds/empty.ts`.
94. Add cross-refs from `02-spec/06-seedable-config-architecture/` to `version.json` as the runtime-config seed.
...
99. On Step 100 completion, move this file from `.ai-memory/plans/pending/16-preview-production-runtime-typed-api.md` to `.ai-memory/plans/completed/16-preview-production-runtime-typed-api.md` and flip `Status:` to `completed`.
100. Close issue `.ai-memory/issues/04-preview-cannot-exercise-ui-without-backend.md` by flipping its status to `resolved` and linking to the completed plan.
...
Existing pending plans in `.ai-memory/plans/pending/` remain independent and are NOT folded into this plan:
