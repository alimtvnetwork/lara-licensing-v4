# Improve 02-spec/21-app from Blind-AI Audit Findings
...
The blind-AI audit under `02-spec/25-app-audit/` sealed at 100.0 Band A but its verdict is not human-readable, and the audit's findings ledger (AF-SU-021, AF-ID-030/031, AF-OBS-050/051, AF-UM-014, AF-ST-001, AF-MIG-040) was resolved by patching downstream artifacts rather than the original `02-spec/21-app/` files. This plan pushes those fixes back into `02-spec/21-app/` so the source spec, not the audit companion, becomes the single normative surface, and binds every clause to `.ai-memory/coding-guidelines.md` and `02-spec/03-error-manage/`.

Captured inputs:
- Command: `.ai-memory/02-spec/commands/02-improve-spec-21-from-audit.md`
- Issue: `.ai-memory/issues/01-audit-verdict-not-human-readable.md`
...
1. Read every file in `02-spec/25-app-audit/` and produce a one-page findings index at `02-spec/25-app-audit/98-findings-index.md` listing AF-ID, severity, resolved-in version, and target file under `02-spec/21-app/`.
2. Author the plain-language verdict page. See ./subtasks/02-spec-21-audit-remediation/SS-01-plain-language-verdict.md.
3. Link the verdict page from `02-spec/21-app/00-overview.md` header and from `README.md` project status line.
4. Reset 02-spec/21-app version drift to a shared 1.0.0 baseline. See ./subtasks/02-spec-21-audit-remediation/SS-04-version-drift-reconciliation.md.
5. Record the version reset in `.ai-memory/memory/history/01-decisions.md`.
6. In `02-spec/21-app/00-overview.md`, add a "Normative sources" section citing `.ai-memory/coding-guidelines.md`, `02-spec/02-coding-guidelines/01-cross-language/15-master-coding-guidelines.md`, and `02-spec/03-error-manage/`.
7. Author `02-spec/21-app/18-error-management-binding.md`. See ./subtasks/02-spec-21-audit-remediation/SS-02-error-manage-mapping.md.
...
15. In `11-api-contracts/00-overview.md`, add a "JSON key casing" clause: all envelope keys and payload keys MUST be PascalCase (cites `.ai-memory/strictly-avoid/sa-031-pascal-case-data.md`).
...
21. Author `02-spec/21-app/21-json-envelope.md` codifying the universal envelope (Success, Data, Error, RequestId, RateLimit) with a Mermaid sequence for a 200 and a 429.
...
38. Author `02-spec/21-app/22-testing-contract.md` listing the minimum Vitest coverage bar (transport, response parser, self-update integrity, retry banner, update banner, refresh 401) and require it in CI.
39. Cross-link `22-testing-contract.md` from `package.json` scripts documentation in `README.md`.
40. In `02-spec/23-app-db/01-schema.md`, add the `UserRole` table with `UserRoleId`, `UserId`, `AppRoleId`, `AssignedAt`, `AssignedBy`, and an FK diagram.
41. In `02-spec/23-app-db/01-schema.md`, add `GRANT`/RLS clauses per `.ai-memory/strictly-avoid/sa-042-server-side-authorization.md` even though the target is Laravel, framed as "authorization model requirements".
42. In `02-spec/23-app-db/01-erd.mmd`, add the `UserRole`, `LicenseCategory`, `LicenseVariation` tables so the ERD matches the schema doc.
43. In `02-spec/24-app-ui-design-system/03-components-and-states.md`, add the `RetryAfterBanner` and `UpdateBanner` components as first-class entries with states and a11y notes.
44. In `02-spec/24-app-ui-design-system/04-responsive-and-accessibility.md`, add the `aria-live="polite"` requirement for rate-limit and update banners.
45. In `02-spec/21-app/97-acceptance-criteria.md` (create if missing), fold all new ACs from steps 9-37 into a numbered list, one AC per line, each citing its source file.
46. In `02-spec/21-app/99-consistency-report.md` (create if missing), regenerate the cross-file consistency check now that steps 1-45 have landed, and require zero open findings.
47. Update `02-spec/25-app-audit/00-final-report.md` "Post-seal amendments" to link the new spec files (18, 21, 22) and the verdict page.
48. Update `.ai-memory/plan.md`: bump version to 0.78.0, mark M3 sub-items introduced by this plan, and add a "Spec remediation" milestone entry.
49. Update `CHANGELOG.md` and `RELEASE-NOTES.md` with a "Spec remediation from blind-AI audit" entry citing this plan.
50. Move this plan from `.ai-memory/plans/pending/02-spec-21-audit-remediation.md` to `.ai-memory/plans/completed/02-spec-21-audit-remediation.md` and flip `Status: pending` to `Status: completed`; do the same for each `SS-*.md` file whose goal is met.

## Verification

- `bun run build:dev` remains green after each spec-only edit (no code touched).
- Every new AC-ID appears in `02-spec/21-app/97-acceptance-criteria.md` and is cited from at least one leaf file.
...
- Final scan: `rg "—" 02-spec/21-app 02-spec/23-app-db 02-spec/24-app-ui-design-system` returns zero matches.

## Appended from prior pending tasks

- `.ai-memory/plans/pending/01-blind-ai-audit-folder-21.md` is sealed at step 50 per `02-spec/25-app-audit/00-final-report.md` and MUST be moved to `.ai-memory/plans/completed/01-blind-ai-audit-folder-21.md` as part of step 50 above.
- `.ai-memory/pending-tasks/task-001-reconcile-spec-numbering.md` remains open and is addressed indirectly by step 4 (version reset); a follow-up task will be filed if numbering issues persist after step 46.
- `.ai-memory/pending-issues/issue-001-spec-numbering-drift.md` remains open; step 4 closes it if the reset holds.
