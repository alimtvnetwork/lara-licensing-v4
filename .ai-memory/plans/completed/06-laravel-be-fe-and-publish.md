...
Implement the Lara Licensing v1 runtime as a Laravel application (backend PHP + Laravel-served Inertia/React frontend), plus a PowerShell script that produces a deployable `publish/` folder, plus the self-update manifest endpoint. Aligns with `02-spec/21-app/*` (API contracts, envelope, ETag, Idempotency-Key, error taxonomy, RBAC, quota, features, environments, impersonation, self-update) and `02-spec/23-app-db/*` (Root DB + per-Reseller shard split-DB).

Captured command: `.ai-memory/02-spec/commands/04-laravel-be-fe-and-publish.md`.
Prior pending plan absorbed: `.ai-memory/plans/pending/05-rbac-quota-tier-environment.md` (runtime layer C residuals rolled into steps below).

## Steps

1. [x] Implement Plan 06 Step 01: Create `docs/laravel-parity/backend-migration-roadmap.md` mapping every table and RLS policy from `02-spec/23-app-db/01-schema.md` to a Laravel migration path.
...
4. [x] Add `config/lara.php` mirroring closed-set enums (Roles, LicenseTier, Environment, LedgerAction, ApiErrorCode) from `02-spec/21-app`.
4. [x] Create `app/Support/ApiEnvelope.php` producing `{Status, Attributes, Results}` responses per `02-spec/21-app/11-api-contracts/05-envelope-schema.md` v1.1.0 (original plan wording `{Ok,Data,Error,Meta}` was stale; corrected to spec).
...
11. [x] Migration `root_0001_resellers.php`: Resellers table with shard DSN columns per `02-spec/23-app-db/10-reseller-shard-split-db.md`.
...
53: 36. [~] Controller `Reseller\QuotaRequestController@index/store/cancel` writing to shard `QuotaRequests` shipped in v0.215.0; Admin approve/deny/adjust land in step 37. Root-inbox mirror deferred: 02-spec/23-app-db/10 §57 says QuotaRequests is shard-native, so no Root table is needed; a global-admin cross-tenant view will be a fanout read in step 37, not a synchronous Root mirror row.
...
56: 39. [x] Controller `Portal\VerifyController@final` POST handshake per `02-spec/21-app/diagrams/licensing-flow.mmd` v1.1. (v0.681.0)
...
77: 60. [x] Feature test `ErrorTaxonomyTest` asserting every thrown `LaraException` matches an entry in `02-spec/21-app/12-error-taxonomy.md`. Shipped in an earlier turn: `backend/tests/Feature/Admin/ApiContractTest.php` error-taxonomy assertions.
...
117: 100. Update `README.md`, `CHANGELOG.md`, `RELEASE-NOTES.md`, `.ai-memory/plan.md`, `02-spec/21-app/98-remaining-work.md`; bump version; move this plan to `completed/`.
...
- Cross-reference: every AC referenced in steps still resolves in `02-spec/21-app/97-acceptance-criteria.md`.
