...
IndexedDB seed rows exist. See screenshot in `.ai-memory/issues/05-seed-mode-audit-load-failure.md`.
...
from the canonical seedable-config surface in `02-spec/06-seedable-config-architecture/`
...
Related:
- Command: `.ai-memory/02-spec/commands/10-seed-mode-must-work-fast-from-config-db.md`
- Issue: `.ai-memory/issues/05-seed-mode-audit-load-failure.md`
- Spec: `02-spec/06-seedable-config-architecture/` (fundamentals, features, acceptance)
...
6. Read every file under `02-spec/06-seedable-config-architecture/` (fundamentals, features/**, issues/**, 97/98/99). Extract the canonical closed-set catalog and feature catalog rows that any seedable config DB must expose.
...
45. Update `.ai-memory/memory/index.md` with a one-liner rule: "Preview mode is the primary dev surface; every route must render green under `default` and `empty` seeds."
46. Update `02-spec/28-runtime-modes/` if any invariants change; otherwise stamp v1.x note that boot is gated.
...
50. Move this plan to `.ai-memory/plans/completed/17-seed-mode-reliability-and-config-db.md`, flip `Status: completed`, close `.ai-memory/issues/05-seed-mode-audit-load-failure.md`.
