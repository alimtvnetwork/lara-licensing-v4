...
Bring the frontend into conformance with `02-spec/24-app-ui-design-system/` (tokens, shell, components, blueprints, motion, a11y, copy) and fine-tune the coding across FE and Laravel BE per `02-spec/02-coding-guidelines/` and `02-spec/03-error-manage/`. Existing prior pending plans: `05-rbac-quota-tier-environment.md`, `06-laravel-be-fe-and-publish.md` (still in progress); this plan is additive and focused on UI polish + code hygiene, not on replacing them.

Related sources read: 02-spec/24-app-ui-design-system/{01,02,03,04,06,07,08,09,10,11,12,13,14,15,16,17..26,27,28,29,30,31,50,51,52,53,54,56,57}, 02-spec/02-coding-guidelines/{02-typescript,04-php,11-security}, 02-spec/03-error-manage/**.

No new commands or issues were provided this turn.

## Steps

1. [DONE v0.247.0] Inventory current `src/styles.css` tokens vs `02-spec/24-app-ui-design-system/08-token-registry.md`; produce a delta list in `.ai-memory/plans/subtasks/07-ui-spec-conformance-and-code-finetune/SS-01-token-delta.md`. See ./subtasks/07-ui-spec-conformance-and-code-finetune/SS-01-token-delta.md
...
47. BE code fine-tune: ensure every service method matches error-manage rules from `02-spec/03-error-manage/**` (no swallowed exceptions, structured logs with request context, taxonomy-mapped `LaraException`).
...
50. Author `02-spec/24-app-ui-design-system/62-plan-07-acceptance-report.md` capturing screenshots, a11y results, and coverage delta, then move this plan to `.ai-memory/plans/completed/07-ui-spec-conformance-and-code-finetune.md` with `Status: completed`.
