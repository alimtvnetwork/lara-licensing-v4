...
Fulfills user command captured in `.ai-memory/02-spec/commands/11-backend-seed-login-e2e-error-manage.md`, driven by the reproduction in `.ai-memory/issues/06-admin-overview-kpis-red-error.md` (screenshot at `.ai-memory/02-spec/tasks/assets/18-backend-seed-login-e2e-error-manage/admin-overview-red-errors.png`). Full intent, scope, and acceptance criteria live in `.ai-memory/02-spec/tasks/18-backend-seed-login-e2e-error-manage.md` (referred to below as "spec-18"). Subtask index at `./subtasks/18-backend-seed-login-e2e-error-manage/README.md`.
...
Normative sources: `02-spec/03-error-manage/`, `02-spec/21-app/`, `02-spec/23-app-db/`, `02-spec/28-runtime-modes/`, `02-spec/02-coding-guidelines/`, `.ai-memory/memory/standards/preview-is-primary-dev-surface.md`.
...
35. Add `FeatureCatalogSeeder` extension covering every entry in `02-spec/21-app/12-feature-catalog.md`.
...
38. Add a `MigrationsAreIdempotentTest` Pest test (deferred item from `.ai-memory/plan.md`) - migrate:fresh --seed twice, assert byte-identical `describe`.
...
47. Add ARIA labels + hotkey `Ctrl+Shift+D` for the demo panel (per `.ai-memory/coding-guidelines.mdcheck-shortcut-registry.py`).
...
54. Add memory note `.ai-memory/memory/standards/seed-mode-demo-login.md` describing the invariant "seed mode never blocks the reviewer".
55. Register the new memory file in `.ai-memory/memory/index.md`.
...
### Phase E - Error-manage backend (spec-18 AC-07, AC-08, AC-09; 02-spec/03-error-manage)

81. Read `02-spec/03-error-manage/02-error-architecture/06-apperror-package/01-apperror-reference/00-overview.md` and every sibling file; summarize the mandated shape into `docs/backend/error-manage-overview.md`.
...
120. Update `.ai-memory/memory/index.md` with a pointer to the new notification standard file `.ai-memory/memory/standards/error-notification-center.md`.
...
174. Add a `plan18-final-gate` workflow that runs only after all subtasks flip to `Status: completed` (checks the file with `rg -c 'Status: completed' .ai-memory/plans/subtasks/18-...`).
...
178. Update `.ai-memory/cicd-index.md` with the new workflows.
...
184. Update `.ai-memory/plan.md` M5/M6 progress lines with the closed items from Plan 18.
185. Update `.ai-memory/memory/index.md` with the two new standards (seed-mode demo login, error notification center).
...
188. Take an "after" screenshot of `/admin` under the `default` seed and save to `.ai-memory/02-spec/tasks/assets/18-backend-seed-login-e2e-error-manage/admin-overview-after.png`.
189. Diff before/after screenshots and record the comparison in the spec-18 acceptance evidence appendix.
190. Confirm every subtask under `.ai-memory/plans/subtasks/18-backend-seed-login-e2e-error-manage/` reads `Status: completed`.
...
196. Move `.ai-memory/plans/pending/18-backend-seed-login-e2e-error-manage.md` to `.ai-memory/plans/completed/18-backend-seed-login-e2e-error-manage.md` and flip `Status: completed` in the same move.
197. Update `.ai-memory/plans/index.md` entry for slug `backend-seed-login-e2e-error-manage` to `completed` with the completion date.
198. Bump the MINOR version per `02-spec/11-release.md` (only touch `package.json`, `public/version.json`, and root `README.md` version pin).
...
200. Run the release ceremony completion checks in `02-spec/11-release.md` and confirm all AC-01..AC-10 in spec-18 are satisfied; Plan 18 is done.
