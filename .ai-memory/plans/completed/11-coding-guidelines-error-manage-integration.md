...
17. Enforce 15-line function body cap in PHP: add a Pest architecture test using PhpParser on `backend/app/`; whitelist only the closed set of exceptions in `02-spec/02-coding-guidelines/04-php/`.
...
19. Fix all violations reported by step 18; commit report to 02-spec/03-error-manage/98-magic-literals-report.md.
...
22. Backfill `02-spec/03-error-manage/03-error-code-registry/` with any codes missing from the enum by scanning grep of throws.
...
37. Add `02-spec/03-error-manage/02-error-architecture/07-logging-and-diagnostics/` runbook: how to grep `lara-diag-*.log` by ErrorId.
...
48. Add CHANGELOG entry to `02-spec/03-error-manage/98-changelog.md` documenting v0.4XX.0 integration bump.
49. Run full CI locally (`bun run check:error-codes && cd backend && ./vendor/bin/pest && cd .. && bun run test && bun run test:e2e:smoke`) and record pass/fail per step in `02-spec/03-error-manage/99-integration-report.md`.
50. Move this plan from `.ai-memory/plans/pending/11-...md` to `.ai-memory/plans/completed/11-...md` and flip `Status: pending` -> `Status: completed`; bump project version and update mem://index.md to reference the new error-contract axis.
...
Pulled from `.ai-memory/plans/pending/` scan (not yet completed):
