...
- Error management: implement per `02-spec/03-error-manage/` (LaraException / ApiErrorCodeType parity, envelope `{Status, Attributes, Results}`, ErrorId + RequestId correlation, notifications on 5xx).
...
- Spec task `.ai-memory/02-spec/tasks/18-backend-seed-login-e2e-error-manage.md`.
- Plan `.ai-memory/plans/pending/18-backend-seed-login-e2e-error-manage.md` with exactly 200 steps.
- Subtask stubs under `.ai-memory/plans/subtasks/18-backend-seed-login-e2e-error-manage/`.
