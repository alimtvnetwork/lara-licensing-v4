...
Scope: `02-spec/21-app` (Lara Licensing) implementation surface.
Captured: 2026-07-18
...
Rules:
- Backend framework: Laravel (PHP), matches `02-spec/21-app` API contracts (envelope, ETag, Idempotency-Key, error taxonomy).
- Frontend: Laravel-served UI (Blade + Inertia + React), reusing existing blueprints in `02-spec/24-ui`.
...
- Self-update endpoint: `GET /Api/SelfUpdate/Manifest` per `02-spec/21-app/17-self-update-endpoint.md` v1.3.0 (Stable channel only for v1.0).
- Split-DB: Root DB + per-Reseller shard, per `02-spec/23-app-db/10-reseller-shard-split-db.md`.
