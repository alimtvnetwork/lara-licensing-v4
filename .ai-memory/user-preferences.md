...
Canonical, always-on rules for how the AI collaborates with the operator of this project. Agents MUST read this file during onboarding (item #4 in `.ai-memory/what-to-read.md`) and apply every rule from turn 1. Preferences here override defaults; they do NOT override safety, correctness, or explicit in-turn instructions from the user.
...
- Never create per-invocation archive/mirror files for dropdown prompts (`xx-next-task.md`, `xx-read-prompt.md`, `xx-proofread.md`, etc.) under `01-prompts/` or any similar folder. Skip the "saved prompt as NN-*.md" step entirely.
...
- Memory folder is `.ai-memory/memory/` (no trailing "s"). Do not create `.ai-memory/memories/`.
