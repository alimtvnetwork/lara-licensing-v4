...
Canonical registry of standing prompts stored under `01-prompts/`.
...
- When adding a prompt, add a row to the table above and reference it from `.ai-memory/what-to-read.md` if it belongs in the read-order.

## Why the archive ban

User standing rule (see `.ai-memory/user-preferences.md` and `.ai-memory/strictly-avoid.md` SA-024). Per-invocation mirrors bloat the directory, duplicate transient text, and were explicitly rejected.
