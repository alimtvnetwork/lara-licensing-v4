/**
 * v0.302.0. Extracted from `src/lib/copy.ts` to break a runtime cycle with
 * `src/lib/lara-api-error.ts`. Root cause of the cycle: `copy.ts` used
 * computed keys `[ApiErrorCodeType.X]` which force a runtime enum
 * evaluation before `errorsByCode` can be built; `lara-api-error.ts` now
 * needs to import that map for `formatLaraApiError`. By using plain
 * string-literal keys here and a `type`-only import of the enum, this
 * module has no runtime dependency on `lara-api-error.ts` and can be
 * imported from it safely.
 *
 * The coverage guard in `tests/error-copy-coverage.test.ts` still enforces
 * bidirectional parity between the enum and this table.
 */
import type { ApiErrorCodeType } from "./lara-api-error";

export const errorsByCode: Readonly<Record<ApiErrorCodeType, string>> = {
  ...
  "Policy.VersionMismatch": "The policy version is not supported by this instance.",
  "Snapshot.Pinned": "This snapshot is pinned and cannot be deleted or pruned.",
};

/**
 * v0.302.0. Look up user-visible copy for an `ApiErrorCodeType`.
 * Interpolates `{RetryAfterSec}` for `RateLimited`. Returns `undefined`
 * only when the code is not a member of the enum (impossible under
 * TypeScript, but defended for wire-level drift).
 */
export function copyForErrorCode(
  code: ApiErrorCodeType,
  context?: { retryAfterSeconds?: number },
): string | undefined {
  const template = (errorsByCode as Record<string, string>)[code];
  if (template === undefined) return undefined;
  if (code === "RateLimited") {
    // Without a Retry-After header we cannot interpolate the placeholder;
    // fall back to the server-provided message so the alert never shows
    // a raw `{RetryAfterSec}` token to end users.
    if (context?.retryAfterSeconds === undefined) return undefined;
