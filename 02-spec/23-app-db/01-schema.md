...
| `ExpiresAt` | DATETIME NOT NULL | Sliding: `IssuedAt + 30d` for `Kind = 1`. For `Kind = 2` capped at `IssuedAt + 30 minutes` and never extended (02-spec/21-app/46-impersonation.md §3). |
...
Indexes: `(UserId, RevokedAt, ExpiresAt)` for session-cap queries, `(FamilyId)` for cascade revocation, `(ImpersonatorUserId, RevokedAt)` filtered on `Kind = 2` to enforce the one-active-impersonation-per-operator invariant (AC-IMP-004) and to power operator-history audit queries (02-spec/21-app/46-impersonation.md §5).
