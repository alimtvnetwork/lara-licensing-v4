...
**AI Confidence:** Draft
**Ambiguity:** Low
**Authoritative refs:** `02-spec/05-split-db-architecture/00-overview.md`, `02-spec/05-split-db-architecture/01-fundamentals.md`, `02-spec/19-main-worker-service/11-split-db-tier-reconciliation.md`
...
Every other table in `02-spec/23-app-db/01-schema.md` moves to the App tier and is created per shard.
...
2. Migration runner creates `data/resellers/{ResellerSlug}/app.db`, runs the App-tier migration template forward-only per `02-spec/04-database-conventions/03-orm-and-views.md`, seeds required closed-set rows (roles, environments, license categories), and sets `ShardStatus=Active`, `SchemaVersion=<current>`.
...
Per `02-spec/21-app/46-impersonation.md` and `01-schema.md` v0.19.0, `AuthSessions` lives in the App tier of the reseller shard where the target user resides. An Admin impersonating a reseller user opens a Session-tier DB scoped to that shard. The Root DB never stores impersonation state.

---

## Migration Discipline

- One migration template, versioned in `02-spec/23-app-db/02-migration-order.md`, is the single source for all shard schemas. Every shard MUST report the same `SchemaVersion`.
...
- `02-spec/05-split-db-architecture/00-overview.md`, tier definitions
- `02-spec/05-split-db-architecture/01-fundamentals.md`, isolation rules
- `02-spec/19-main-worker-service/11-split-db-tier-reconciliation.md`, 4-tier framing
- `02-spec/23-app-db/01-schema.md`, table columns and constraints
- `02-spec/23-app-db/02-migration-order.md`, migration numbering
- `02-spec/21-app/46-impersonation.md`, AuthSessions Kind field
- `.ai-memory/strictly-avoid/sa-031-pascal-case-data.md`, naming rule
