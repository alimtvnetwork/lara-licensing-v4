...
Version: 2.0.0
Updated: 2026-07-19
Owning plan mirror: `.ai-memory/plans/pending/11-audit-v2-remediation-300-steps.md` (to be created next session).
...
Legend for target file: `BE` = `backend/`, `FE` = `src/`, `SPEC` = `02-spec/`, `CI` = `.github/workflows/` or `linter-scripts/`, `DOCS` = `docs/`.

## Group 1 - Spec fidelity (steps 1-10)

1. SPEC: diff `02-spec/21-app/17-self-update-endpoint.md` vs `02-spec/14-update/01-self-update-overview.md`; capture delta in `02-spec/25-app-audit/A1-diff.md`.
...
4. SPEC: cross-link 17 into `02-spec/21-app/00-overview.md` under "Endpoints".
5. SPEC: add ACs `AC-SU-1..8` to `02-spec/25-app-audit/97-acceptance-criteria.md` (recreate section).
...
8. SPEC: rename any PascalCase field in `02-spec/23-app-db/*` that disagrees with `02-spec/21-app/11-api-contracts/*`.
9. SPEC: re-run `linter-scripts/ac-index-parity.py` and pin to green.
10. SPEC: add `02-spec/25-app-audit/A2-field-name-audit.md` with a table of every DB column vs its JSON key.
...
216. DOCS: extend `02-spec/21-app/19-user-management.md` with an "approval matrix" (who can approve what).
