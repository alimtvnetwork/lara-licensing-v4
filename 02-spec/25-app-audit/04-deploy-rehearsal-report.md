...
`02-spec/21-app/18-publishing-powershell.md` defines the contract, but there is no e2e spec exercising the full mint-upload-manifest-verify loop. `admin-app-updates.spec.ts` does not exist. The 9560 to 9569 exit codes are unproved.
...
- Feeds `02-spec/25-app-audit/02-gap-catalog.md` gaps: deploy blockers, RBAC-adjacent (F12).
- Feeds `02-spec/25-app-audit/03-plan-300-steps.md` steps 283 to 300.
