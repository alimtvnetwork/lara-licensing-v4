...
- `Attributes.Code`: `RUNTIME_CONFIG_INVALID` (to be registered in `02-spec/03-error-manage/03-error-code-registry` at Step 58 landing).
...
- `02-spec/28-runtime-modes/00-overview.md`: INV-RM-01..INV-RM-10 pinning the fields formalized here.
- `02-spec/28-runtime-modes/02-mode-selection-precedence.md` (Step 3, pending): consumes `Mode` and `ApiBaseUrl`.
- `02-spec/28-runtime-modes/05-admin-runtime-toggle.md` (Step 6, pending): the write path that MUST validate against this schema.
- Plan 16 Steps 9 (create root `version.json`), 11 (`check-version-json.py`), 15 (`version-json-loader.ts`), 58 (`RuntimeConfigController`).
- `02-spec/03-error-manage/`: envelope shape and error-code registry.
- `02-spec/04-database-conventions/`: PascalCase JSON keys rule (same convention applied here).
