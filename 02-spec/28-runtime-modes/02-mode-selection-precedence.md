...
- **Rule P-03.** `version.json` is honored only when the fetch resolves to a body that validates against `02-spec/28-runtime-modes/01-version-json-schema.md`. Any parse or schema failure logs to `console.error` with a `LaraException`-shaped payload (`Code: RUNTIME_CONFIG_LOAD_FAILED`, `RequestId`, `ErrorId`) and falls through.
...
- **Codes emitted:** `RUNTIME_CONFIG_LOAD_FAILED`, `RUNTIME_OVERRIDE_INVALID`, `STORAGE_WRITE_FAILED`. To be registered in `02-spec/03-error-manage/03-error-code-registry` when Step 13 lands.
...
- `02-spec/28-runtime-modes/00-overview.md`: INV-RM-01, INV-RM-08, INV-RM-10.
- `02-spec/28-runtime-modes/01-version-json-schema.md`: the schema P-03 validates against.
- `02-spec/03-error-manage/`: envelope + error-code registry (codes above).
