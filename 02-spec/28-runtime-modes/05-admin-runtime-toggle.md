...
Pin the contract for the runtime-config toggle surface (backend endpoint + Admin UI) that mutates the deployed `version.json` at runtime. Every mutation MUST go through this contract, MUST be gated by RBAC, MUST use optimistic concurrency on `UpdatedAt`, and MUST emit an audit event that is byte-parity with `02-spec/03-error-manage` conventions.
...
Every code MUST be present in `02-spec/03-error-manage/03-error-code-registry` before Step 58 lands; failure to register fails the closed-set parity check.
...
- **T-03.** Contract test: envelope codes match `02-spec/03-error-manage/03-error-code-registry` (parity linter).

## Cross-References

- `02-spec/28-runtime-modes/00-overview.md`: INV-RM-06, INV-RM-07, INV-RM-08.
- `02-spec/28-runtime-modes/01-version-json-schema.md`: mutable field surface, write discipline.
- `02-spec/28-runtime-modes/02-mode-selection-precedence.md`: how the FE re-resolves after a successful write.
- `02-spec/28-runtime-modes/04-generated-types-contract.md`: `Operations["putAdminRuntimeConfig"]` typing.
- `02-spec/03-error-manage/03-error-code-registry`: closed-set code additions.
- Plan 16 Steps 58-64.
