...
| Criterion | Status |
|-----------|--------|
| `00-overview.md` present in module | ✅ (`02-spec/28-runtime-modes/00-overview.md`) |
...
- **B-02.** The hand-written file carries the same `// @generated` banner and the extra line `// baseline: hand-authored per 02-spec/28-runtime-modes/04-generated-types-contract.md §Hand-Written Baseline; MUST be replaced by generator output at Step 30 landing`.
...
- **O-03.** `Response` is the success envelope shape from `02-spec/28-runtime-modes/03-preview-fixture-contract.md §Envelope Shape`.
...
- Generator crash: script exits with the underlying error and a `RUNTIME_TYPES_GEN_FAILED` prefix; no partial file is written (write to `.tmp` then rename atomically, mirroring `version.json` write discipline from `02-spec/28-runtime-modes/01-version-json-schema.md §Location`).

## Cross-References

- `02-spec/28-runtime-modes/00-overview.md`: INV-RM-04.
- `02-spec/28-runtime-modes/03-preview-fixture-contract.md`: envelope shape used by `Operations["*"]["Response"]`.
- `02-spec/03-error-manage/03-error-code-registry`: closed set of error codes referenced by `ErrorCodes` unions.
- `02-spec/02-coding-guidelines/`: PascalCase JSON keys, no magic strings.
