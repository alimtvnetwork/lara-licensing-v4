...
**Version:** 1.0.0
**Status:** Normative for LaraLicensingV1. Single normative source for the OpenAPI 3.1 document that describes every route in `02-spec/21-app/26-route-dto-index.md`. Any runtime API surface change MUST update this contract in the same commit.
...
The OpenAPI document is authored as a set of YAML fragments under `02-spec/api/`:
...
- Every fragment carries a top-of-file comment citing its normative section in `02-spec/21-app/`; drift between the fragment and its normative source is a lint failure.
...
securitySchemes:
  BearerAuth:
    ...
    bearerFormat: JWT
    description: |
      JWT bearer per 02-spec/21-app/02-authentication-jwt.md.
      Session families and rotation described in 02-spec/21-app/31-auth-session-family.md.
  IdempotencyKey:
    type: apiKey
    in: header
    name: Idempotency-Key
    description: |
      Required on every non-idempotent operation per 02-spec/21-app/29-idempotency-lifecycle.md.
...
Every 2xx response body is a `Response<T>` envelope per `02-spec/21-app/11-api-contracts/05-envelope-schema.md`:
...
- Property casing: PascalCase, matching the DB and JSON conventions from `.ai-memory/strictly-avoid.md`.
      ...
      properties:
        ...
        Details:
          ...
          description: |
            ...
            documented in 02-spec/21-app/12-error-taxonomy.md.
        RetryAfterSec: { type: integer, minimum: 0, nullable: true }
    Meta: { $ref: '#/components/schemas/ResponseMeta' }
ErrorCode:
  type: string
  enum: # generated from 02-spec/21-app/12-error-taxonomy.md; drift is a lint failure
    - AuthFailed
    - Unauthorized
    - Forbidden
    - LicenseNotFound
    ...
    # ... closed set; see 02-spec/21-app/12-error-taxonomy.md
```

- `ErrorCode` is a closed enum GENERATED at bundle time from `02-spec/21-app/12-error-taxonomy.md`; a mismatch between the two files is a lint failure per `check-error-code-parity.py` (§13). Hand-maintaining the enum in both files is BANNED.
...
- `enum` order MUST match the normative source ordinal (per `.ai-memory/strictly-avoid.md` rule 24 pinning ordinal stability); reordering is a lint failure.
...
- `x-blueprint-ref` extension citing the route blueprint under `02-spec/24-app-ui-design-system/` that owns the UX for this operation (e.g. `x-blueprint-ref: 02-spec/24-app-ui-design-system/34-route-blueprint-admin-licenses.md`). Operations without a UI (webhooks, cron) omit this extension.
...
1. Operation in `02-spec/api/paths/*.yaml` without a matching row in `26-route-dto-index.md`.
...
7. Blueprint route in `02-spec/24-app-ui-design-system/*route-blueprint*.md` with no `x-blueprint-ref` back-pointer from at least one operation.
...
- AC-OPENAPI-001: `redocly bundle 02-spec/api/openapi.yaml -o dist/openapi.json` succeeds with zero warnings.
...
- SDK generation (TypeScript, Python, Go, C#) deferred to a `02-spec/api/generators/*.yaml` config set.
