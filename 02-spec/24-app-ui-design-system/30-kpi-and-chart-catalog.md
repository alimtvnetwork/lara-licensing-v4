...
`operationId` values are provisional and MUST be reconciled with `../21-app/26-route-dto-index.md` and the future `02-spec/26-app-api-swagger/` folder.
...
- Manual: §7 registry rows are a strict subset of the operationIds that will appear in `02-spec/26-app-api-swagger/` (future); parity check is deferred to that folder's AC.
