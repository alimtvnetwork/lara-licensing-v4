...
- Runtime: the Worker generates the PDF via a pure-JS library that ships as WASM or plain JS (do NOT introduce Node-only `puppeteer` or `sharp` per the server runtime constraints). Candidate library `pdf-lib` (pure JS, no native deps) is the default; final selection recorded in `.ai-memory/decisions/pdf-library.md` when the runtime task lands.
...
- Verifies every `.csv` / `.json` / `.pdf` export endpoint in `02-spec/api/paths/*.yaml` carries the `x-csv-columns` / `x-json-envelope` / `x-pdf-template` extension per §9.
