...
## 3. Personas (mapped to `02-spec/21-app/16-ui-surfaces.md`)
...
1. Status meaning never depends on color alone; every status carries an icon or a label from `02-spec/21-app/15-license-lifecycle.md`.
2. Every mutation is idempotent-key protected and reports `RequestId` in an Advanced disclosure.
3. Every error surface names the canonical `ErrorCode` from `02-spec/21-app/12-error-taxonomy.md`.
