...
- `Verify by serial...` (products list): opens the Verify Dialog. Permission `Me.Products.Verify`. Server POST to the end-user verify endpoint per the current verify-handshake decision (POST, per `02-spec/21-app/03-authentication-oauth.md` and Issue 03 closure); the flow MUST match `02-spec/21-app/diagrams/licensing-flow.mmd`.
