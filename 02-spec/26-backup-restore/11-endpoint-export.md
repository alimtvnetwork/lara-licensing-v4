...
- Envelope shape is the canonical response envelope from `02-spec/03-error-manage/` (Result, Data, Attributes).
...
Every failure uses the canonical envelope from `02-spec/03-error-manage/`:
...
| `INV-BR-EP-EX-5`  | Every response body conforms to the canonical envelope from `02-spec/03-error-manage/`; `Attributes.Capability` is always `Backup.Export`. |
