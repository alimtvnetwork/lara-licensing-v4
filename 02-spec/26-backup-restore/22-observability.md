...
[`02-spec/03-error-manage/`](../03-error-manage/00-overview.md) and
...
`02-spec/03-error-manage/` and add the following typed fields.
...
`redactor.pii` from `02-spec/03-error-manage/`):
...
| INV-BR-OB-9     | Log level policy MUST match the table in this file; a `warn` emitted at `info` level is a lint failure enforced by the log-level linter cited in `02-spec/coding-guidelines/`. |
...
- [`02-spec/03-error-manage/`](../03-error-manage/00-overview.md): base `RequestId`/`ErrorId`/`ErrorCode` chain, redactor implementations.
