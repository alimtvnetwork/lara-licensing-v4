...
5. `.ai-memory/plans/pending/13-*.md` is moved to `.ai-memory/plans/done/` (Plan 14 handles this at commit time).
...
- Every file in `02-spec/26-backup-restore/` from `00-overview.md` through `26-runbooks.md`.
- [`02-spec/25-app-audit/03-plan-300-steps.md`](../25-app-audit/03-plan-300-steps.md): Band A+ track that consumes BR module invariants.
- [`.ai-memory/plans/pending/13-*`](../../.ai-memory/plans/pending/): Plan 13 tracking; moved to `done/` at Plan 14 kickoff.
