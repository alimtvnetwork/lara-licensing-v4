...
All requests and responses use the canonical `ApiEnvelope` shape from `02-spec/03-error-manage/`. All error surfaces route through `LaraException` and are logged via the `lara-diag` channel with `RequestId` + `ErrorId` propagation per `22-observability.md`.
