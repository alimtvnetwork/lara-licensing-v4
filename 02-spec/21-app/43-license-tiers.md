...
Every occurrence of a forbidden synonym anywhere under `02-spec/`, `src/`, `linter-scripts/`, `.ai-memory/`, or migrations is a consistency defect and MUST be caught by `linter-scripts/ac-index-parity.py`'s sibling check for vocabulary (extension tracked by AC-LT-004 below).
...
- **AC-LT-004** The forbidden-synonym set in §2 is enforced by a linter check that scans `02-spec/`, `src/`, `linter-scripts/`, `.ai-memory/`, and every migration file in the repository for the exact tokens listed and fails the build if any occurrence is found outside this file and [`99-consistency-report.md`](./99-consistency-report.md); the linter MUST report the file path and line number for each hit.
