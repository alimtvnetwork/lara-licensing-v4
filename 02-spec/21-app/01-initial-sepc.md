...
6. Follow `.ai-memory/coding-guidelines.md` strictly (functions under 8 lines, no nested ifs, no `any`, no swallowed errors, files under 80-100 lines, no magic strings/numbers, Booleans prefixed with `is`/`has`, DRY, small reusable components).

## Plan (50 Steps)

### Phase 1: First 10 Steps (Planning What To Do)

1. Read `.ai-memory/coding-guidelines.md`, `.ai-memory/what-to-read.md`, root `README.md`, and any `/02-spec/coding-guideline/` and `/02-spec/error-manage/` folders.

2. Create spec folder `02-spec/21-lara-licensing-v4/` and diagram folder `02-spec/23-lara-licensing-diagrams/`.
...
29. Write error-response envelope, error codes, and error taxonomy (per `/02-spec/error-manage/` if present).
...
36. Draw ERD Mermaid diagram in `02-spec/23-lara-licensing-diagrams/erd.mmd`.
...
Read `.ai-memory/coding-guidelines.md`, `.ai-memory/what-to-read.md`, and root `README.md`. Follow Boolean, Enum, and error-management guidelines. Every catch must be logged. No `any`, `unknown`, or wide types. Files under 80-100 lines. Booleans prefixed with `is`/`has`. Variables assigned once (Rust-style). Assets under `/assets/xx-folder-name/xx-file-name.ext`.
...
1. Coding tasks (especially Golang, Python, PHP, or other backend). Check ALL three locations:
   ...
   - Error-management folder (MANDATORY for coding tasks). It lives inside a `02-spec`/guidelines folder and is a folder of multiple files ,  it can be named anything but will live under one of these. Check ALL these locations and read every file inside any folder you find:
     ...
     - `02-spec/XX-error-manage/` (e.g. `02-spec/01-error-manage/`) ,  folder; read every file inside.
