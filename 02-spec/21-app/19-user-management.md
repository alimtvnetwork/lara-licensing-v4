...
Singular PascalCase table name per `02-spec/23-app-db/` singularization rule (routed to plan step 32). Reseller row scope is a separate column on `User` (`ResellerId nullable`); it is a data attribute, not a role.
...
Every role check MUST additionally emit a structured log `{RequestId, UserId, Role, Decision}` regardless of audit destination; `X-Request-Id` correlation is mandatory per `02-spec/25-app-audit/16-coding-guideline-alignment.md` AF-CG-013.
...
## Migration path (routed to plan step 32; `02-spec/23-app-db/` update)
...
- Error taxonomy: `02-spec/21-app/12-error-taxonomy.md`.
- Audit log: `02-spec/21-app/13-audit-logging.md`.
- Bridge decisions: `02-spec/25-app-audit/15-user-management-consolidated.md`.
- DB schema: `02-spec/23-app-db/01-schema.md`.
