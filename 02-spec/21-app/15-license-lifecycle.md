...
Driven by `LicenseCategories.DurationSeconds` (see `02-spec/23-app-db/01-schema.md` `LicenseCategories`). Closed category set: `Daily`, `Weekly`, `Monthly`, `Yearly`, `Lifetime`, `Dev`, `Key`.
...
Follow-up: `LicenseCategories` currently has no duration column. `02-spec/23-app-db/01-schema.md` needs a schema addendum before this rule is implementable. Not fixed in this file to keep the change minimum.
