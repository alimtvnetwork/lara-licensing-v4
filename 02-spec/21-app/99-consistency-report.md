...
- Forbidden-synonyms sweep: no file under `02-spec/21-app/` uses `Tier1..Unlimited` to name an environment, nor `Production/Staging/Development` to name a tier, nor `Dev` to describe an environment (beyond the AC-CAT-004 disambiguation stanza in `05-license-categories.md`).
...
- Actor ordering rule declared in `02-spec/21-app/diagrams/00-diagram-contract.md` v1.0.0 (AC-DG-001); referenced from `.ai-memory/02-spec/commands/03-diagram-actor-ordering.md`.
- `02-spec/21-app/diagrams/licensing-flow.mmd` declares participants EndUser, Reseller, Admin, API, DB, Audit (canonical order); Verify is now section 1 (primary path).
- Sibling sweep: `02-spec/23-app-db/02-jwt-flow.mmd` (Client leftmost), `02-spec/23-app-db/03-oauth-client-credentials.mmd` (AppBuilder leftmost), `02-spec/23-app-db/09-verify-sequence.mmd` (WinApp leftmost) all comply and carry an inline compliance header citing AC-DG-001. `02-spec/23-app-db/01-erd.mmd` is an ER diagram, rule not applicable.
- JSON companion: `02-spec/21-app/diagrams/licensing-flow.json` exists, `Source` field points at the `.mmd`, `Actors[].Column` matches the participant order 1..6 (AC-DG-002).
- Verify-path drift between `licensing-flow.mmd` (GET /Serials/{SerialValue}) and the legacy verify diagrams (POST /Verify/Serial) captured in `.ai-memory/issues/03-verify-path-drift.md` and cross-referenced from the affected `.mmd` headers. Not a diagram-contract violation; tracked as a separate contract question.
...
Cross-file consistency check across `02-spec/21-app/` (36 files), `02-spec/23-app-db/` (5 files), and `02-spec/24-app-ui-design-system/` (referenced only). Every check below MUST evaluate green; a red result reopens the plan.

## Automated checks

1. Em-dash sweep: `rg ", " 02-spec/21-app 02-spec/23-app-db 02-spec/24-app-ui-design-system` returns zero matches. GREEN.
2. AC coverage: every AC-* discovered by `rg -o "AC-[A-Z]+-[0-9]+" 02-spec/21-app/ 02-spec/23-app-db/ | sort -u` (190 IDs) appears in `02-spec/21-app/97-acceptance-criteria.md`. GREEN.
...
7. Vocabulary normalization: `24-vocabulary-normalization.md` term set has zero collisions with legacy strings under `02-spec/21-app/` (checked via the canonical replacement table). GREEN.
...
9. Version-drift baseline: every top-level `02-spec/21-app/*.md` file declares `Version: 1.x.y` and no file remains at pre-baseline versions. GREEN.
...
Re-run this report whenever any `02-spec/21-app/*.md` or `02-spec/23-app-db/*.md` file changes. Bump `Version:` above with each regeneration. If any check turns red, open a new plan under `.ai-memory/plans/pending/` and DO NOT edit this file to hide the finding.
