...
4. On match: emit `UpdateVerified` audit event, then proceed with the rename-first-deploy sequence per `02-spec/14-update/03-rename-first-deploy.md`, then emit `UpdateDownloaded`.
...
| A10 | Rename-first-deploy step itself fails after verification (source binary already moved) | `UpdateDeployFailed { Stage }` per `02-spec/14-update/03-rename-first-deploy.md` rollback | no; MUST restore backup |
...
6. **Platform coverage.** `POST /Admin/AppUpdates` MUST reject requests whose `Assets[]` set does not equal the declared coverage set for the `Channel` (see `02-spec/21-app/16-ui-surfaces.md` §Publishing). Partial-platform publishes are a spec break; admins publish all supported platforms atomically.
...
- Client obligations: `02-spec/14-update/` (probe, checksum, rename-first-deploy, handoff, config-file).
- Publish path: `02-spec/21-app/18-publishing-powershell.md` (plan step 24, next).
- Envelope: `02-spec/21-app/11-api-contracts/00-overview.md`.
- Error taxonomy: `02-spec/21-app/12-error-taxonomy.md` (plan step 29 adds five `UPDATE_*` codes).
- Bridge spec: `02-spec/25-app-audit/17-self-update-integration-plan.md`.
...
Normative wire-level sequence for a conforming CLI (`lara update`) once the server has a `Published` row for `(Product, Channel, Version)`. Named states in participant notes refer to the server-side `AppUpdates` row state defined in §"Publish state machine"; the CLI never observes `Draft` or `Staged`. Cross-refs: client-side strategies live in `02-spec/14-update/01-self-update-overview.md`, rename-first-deploy details in `02-spec/14-update/03-rename-first-deploy.md`, handoff mechanism in `02-spec/14-update/05-handoff-mechanism.md`, MUST-abort rows A1..A10 in this file §"MUST-abort table".
