...
- Response bodies **never** contain a `Trace` key. If you see one in a response, that is a bug: open an issue against 02-spec/03-error-manage §4.3.
...
| `ErrorCode` | Closed-set from `02-spec/03-error-manage/03-error-code-registry/` | Only on handled `lara.exception.trace`. |
...
If the stack entry has `Details`, those fields have already passed through `DetailsRedactor::redact` (Plan 11 step 33), so sensitive values are masked in place. Do **not** re-enable raw logging to "see the real value": the redaction is contractual (02-spec/03-error-manage §4.2, AC-ERR-005).
...
- `02-spec/03-error-manage/02-error-architecture/07-logging-and-diagnostics/00-overview.md`: channel design.
- `02-spec/03-error-manage/02-error-architecture/05-response-envelope/`: shape guarantees for `ErrorId` and absence of `Trace`.
